public class STB {
    ...
    public static void runTests() {
        ...
        String[] options0 = { "01 20" }; // 1
            ...
            // show main menu
        /*System.out.println(Display.main_menu(""));
            // show level menu
        System.out.println(Display.level_menu(""));*/

            // show in game example ({-0-})
        System.out.println(Display.in_game(sum(board0), 0, board0, dice0, options0, ""));
            // show in game example ({-1-})
        System.out.println(Display.in_game(sum(board1), 1, board1, dice1, options1, ""));
            // show in game example ({-2-})
        System.out.println(Display.in_game(sum(board2), 2, board2, dice2, options2, ""));
            // show in game example ({-3-})
        System.out.println(Display.in_game(sum(board3), 3, board3, dice3, options3, ""));
            // show in game example ({-4-})
        System.out.println(Display.in_game(sum(board4), 4, board4, dice4, options4, ""));
            // show in game example ({-5-})
        System.out.println(Display.in_game(sum(board5), 5, board5, dice5, options5, ""));

            // show end example (win)
        /*System.out.println(Display.end(0, 5, ""));  
            // show end example (lose)
        System.out.println(Display.end(201, 5, ""));  
            // show leader board example
        System.out.println(Display.leader_board(top10, ""));
            // show info (all)
        for (int i = 0; i <7 ; i++) {// including the 404 page
            System.out.println(Display.info(i, ""));
        }*/
    }
}
